...
class ModelArguments:
    ...
    ##########################
    num_bits: int = field(
        default=4, metadata={"help": "quantization bit"}
    )
    num_iter: int = field(
        default=0, metadata={"help": "The number of iterations during the alternative optimization. 0: native lora"}
    )
    reduced_rank: int = field(
        default=8, metadata={"help": "The rank of lora"}
    )
    path_to_model_zoo: str = field(
        default='/mnt/t-qingru/yixiaoli_model_zoo', metadata={"help": "path to model zoo"}
    )
...
def get_accelerate_model_svd(args, checkpoint_dir):
...
def train():
    ...
    model, tokenizer = get_accelerate_model_svd(args, checkpoint_dir)
    ...
    if args.do_mmlu_eval:
        ...
        class MMLUEvalCallback(transformers.TrainerCallback):
            def on_evaluate(self, args, state, control, model, **kwargs):
                ...
                results[f'mmlu_{args.mmlu_split}_accuracy_{args.seed}'] = np.mean(subject_scores)
                trainer.log(results)
    ...
    if args.do_train:
        ...
        for n,m in model.named_parameters():
            print(n, m.dtype, m.requires_grad)
            if "layers.0" in n:
                print(m)
        logger.info("*** Train ***")
